// Ferramenta: Mover Geometria (Mapa) - v2.04
// Usa sistema de camada ativa + drag and drop com logs detalhados
console.log('🔄 [v2.04] Iniciando carregamento de Mover Geometria (Mapa)...');
...
window.openMoverGeometriaMapaDialog = function() {
    console.log('[MOVER v2.04] Ativando ferramenta Mover Geometria (Mapa)');
    ...
    try {
        ...
        console.log('[MOVER v2.04] Usando camada ativa:', layerName);
        console.log('[MOVER v2.04] geometriaSelecionada:', geometriaSelecionada);
        ...
        console.log('[MOVER v2.04] Preview layer criado e adicionado ao mapa');
        ...
        console.log('[MOVER v2.04] Anexando eventos ao mapa');
        ...
        console.log('[MOVER v2.04] Ferramenta ativada! Clique no mapa para mover.');
        showMessage('🗺️ Clique e arraste no mapa para mover o polígono. ESC para cancelar.', 'info');
        
    } catch (error) {
        console.error('[MOVER v2.04] ❌ ERRO ao ativar ferramenta:', error);
        console.error('[MOVER v2.04] Stack:', error.stack);
...
function onMouseDown(e) {
    if (!moverGeometriaMapaAtivo || !geometriaSelecionada) {
        console.log('[MOVER v2.04] MouseDown ignorado - ferramenta não ativa');
        return;
    }
    
    console.log('[MOVER v2.04] 🖱️ MouseDown - iniciando arrasto');
    console.log('[MOVER v2.04] e.latlng:', e.latlng);
    console.log('[MOVER v2.04] e.containerPoint:', e.containerPoint);
    ...
    if (geometriaSelecionada.geometryLayer) {
        console.log('[MOVER v2.04] Escondendo polígono original');
        geometriaSelecionada.geometryLayer.setStyle({opacity: 0, fillOpacity: 0});
    }
    
    // Trazer preview para frente
    if (previewLayer) {
        console.log('[MOVER v2.04] Trazendo preview para frente');
        previewLayer.bringToFront();
    }
    ...
    console.log('[MOVER v2.04] Arrasto iniciado com sucesso');
}
...
function onMouseMove(e) {
    ...
    try {
        ...
        console.log('[MOVER v2.04] 🔄 MouseMove - dLat:', dLat.toFixed(8), 'dLng:', dLng.toFixed(8));
        ...
        console.log('[MOVER v2.04] ✅ Preview atualizado');
        
    } catch (error) {
        console.error('[MOVER v2.04] ❌ Erro no mousemove:', error);
        console.error('[MOVER v2.04] Stack:', error.stack);
    }
}
...
function onMouseUp(e) {
    if (!moverGeometriaMapaAtivo || !geometriaSelecionada || !arrastando) {
        console.log('[MOVER v2.04] MouseUp ignorado - não está arrastando');
        return;
    }
    
    console.log('[MOVER v2.04] 🖱️ MouseUp - finalizando movimento');
    
    try {
        var pontoFinal = e.latlng;
        
        console.log('[MOVER v2.04] Ponto inicial LatLng:', pontoInicial);
        console.log('[MOVER v2.04] Ponto final LatLng:', pontoFinal);
        console.log('[MOVER v2.04] Fuso da geometria:', geometriaSelecionada.fuso);
        
        var pontoInicialUTM = latLngToUTM(pontoInicial.lat, pontoInicial.lng, geometriaSelecionada.fuso);
        console.log('[MOVER v2.04] ✅ Ponto inicial UTM:', pontoInicialUTM);
        
        var pontoFinalUTM = latLngToUTM(pontoFinal.lat, pontoFinal.lng, geometriaSelecionada.fuso);
        console.log('[MOVER v2.04] ✅ Ponto final UTM:', pontoFinalUTM);
        
        var dx = pontoFinalUTM.e - pontoInicialUTM.e;
        var dy = pontoFinalUTM.n - pontoInicialUTM.n;
        
        console.log('[MOVER v2.04] Deslocamento: dx=', dx.toFixed(2), 'm, dy=', dy.toFixed(2), 'm');
        
        geometriaSelecionada.vertices.forEach(function(v) {
            console.log('[MOVER v2.04] Movendo vértice', v.name, 'de E:', v.e.toFixed(2), 'N:', v.n.toFixed(2));
            v.e += dx;
            v.n += dy;
            console.log('[MOVER v2.04] Para E:', v.e.toFixed(2), 'N:', v.n.toFixed(2));
        });
        ...
        console.error('[MOVER v2.04] ❌ ERRO ao mover:', error);
        console.error('[MOVER v2.04] Stack:', error.stack);
        finalizarMoverGeometriaMapa();
...
function onKeyDownMover(e) {
    if (e.key === 'Escape' && moverGeometriaMapaAtivo) {
        console.log('[MOVER v2.04] ESC pressionado - cancelando');
        finalizarMoverGeometriaMapa();
...
function finalizarMoverGeometriaMapa() {
    console.log('[MOVER v2.04] Finalizando ferramenta');
    ...
    console.log('[MOVER v2.04] Ferramenta finalizada');
}
...
console.log('✅ [v2.04] Ferramenta Mover Geometria (Mapa) carregada!');
